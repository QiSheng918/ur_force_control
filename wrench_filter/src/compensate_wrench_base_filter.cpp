...
void WrenchsubCallback(const geometry_msgs::WrenchStamped& msg)
{
    ...
    for(int i=0;i<3;i++){
        if(abs(wrench_sum[i])<2*frequency) wrench_sum[i]=0;
    }
    for(int i=3;i<6;i++){
...
{
    ros::init(argc, argv, "compensate_wrench_base_filter");
    ros::NodeHandle nh;
    ...
    wrench_pub = nh.advertise<geometry_msgs::WrenchStamped>("/compensate_wrench_base_filter", 1000);
