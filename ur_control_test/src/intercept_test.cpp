...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if (flag == 0)
        {
            ...
            path->Add(KDL::Frame(KDL::Rotation::Quaternion(0.586656,0.587972,-0.340895,-0.440356), KDL::Vector(-0.23073, -0.0927758, 0.647443)));
    ...
    while (nh.ok())
    {
        ...
        for (int i = 0; i < 3; i++)
            // command_vel[i+3] = target_vel.rot.data[i] + 0.5 * (target_pose(i, 3) - current_pose[i]);
            command_vel[i+3] = target_vel.rot.data[i];
